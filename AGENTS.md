...
Mognet is a minimal web GUI for using coding agents like Codex and Claude.
...
- In a linked git worktree, dev state defaults to that worktree's gitignored `.mognet`. This deliberately outranks an ambient `MOGNET_HOME`, which could otherwise select the installed app's live `~/.mognet/userdata` database. An explicit `--home-dir` still wins.
