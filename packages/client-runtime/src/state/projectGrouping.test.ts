...
const repositoryIdentity = {
  canonicalKey: "github.com/t3tools/mognet",
  locator: {
    source: "git-remote" as const,
    remoteName: "upstream",
    remoteUrl: "https://github.com/t3tools/mognet.git",
  },
  provider: "github",
  owner: "t3tools",
  name: "mognet",
...
describe("buildProjectGroups", () => {
  it("preserves every physical clone as a selectable member in repository modes", () => {
    const projects = [
      ...
      makeProject("mognet-2", "/work/mognet-2"),
      makeProject("mognet-3", "/work/mognet-3"),
    ];
  ...
  it("keeps physical clones in separate groups when requested", () => {
    ...
    expect(groups.map((group) => group.label)).toEqual(["mognet", "mognet-2", "mognet-3"]);
  });

  it("applies a physical-project override without dropping its siblings", () => {
    const first = makeProject("mognet", "/work/mognet");
    const second = makeProject("mognet-2", "/work/mognet-2");
    const third = makeProject("mognet-3", "/work/mognet-3");
  ...
  it("dedupes stale registrations at one physical path using the freshest project", () => {
    const stale = makeProject("stale", "/work/mognet", {
      repositoryIdentity: null,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const fresh = makeProject("fresh", "/work/mognet/", {
  ...
  it("uses repository identity from a duplicate registration when the winner lacks it", () => {
    const identified = makeProject("identified", "/work/mognet", {
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshUnidentified = makeProject("fresh", "/work/mognet/", {
      repositoryIdentity: null,
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const sibling = makeProject("sibling", "/work/mognet-2");
  ...
  it("uses the freshest identity-bearing duplicate when the winner lacks identity", () => {
    ...
    const staleIdentified = makeProject("stale-identified", "/work/mognet", {
      repositoryIdentity: staleIdentity,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshIdentified = makeProject("fresh-identified", "/work/mognet/", {
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const winner = makeProject("winner", "/work/mognet", {
      repositoryIdentity: null,
