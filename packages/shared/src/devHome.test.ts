...
import { resolveGitWorktreePath, resolveWorktreeMognetHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "mognet-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeMognetHome", () => {
  it.effect("answers with .mognet before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeMognetHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".mognet"));
