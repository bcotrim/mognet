...
const SSH_ASKPASS_DIR_NAME = "mognet-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when Mognet re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# MOGNET_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${MOGNET_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$MOGNET_SSH_AUTH_SECRET"
  exit 0
fi
printf 'Mognet ssh-askpass invoked without MOGNET_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Mognet re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if MOGNET_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:MOGNET_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:MOGNET_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("Mognet ssh-askpass invoked without MOGNET_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "mognet-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(input.authSecret === undefined ? {} : { MOGNET_SSH_AUTH_SECRET: input.authSecret ?? "" }),
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "mognet" }),
  };
});
