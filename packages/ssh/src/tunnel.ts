...
remote_node_satisfies_engine() {
  MOGNET_NODE_ENGINE_RANGE=@@MOGNET_NODE_ENGINE_RANGE@@
  if [ -z "$MOGNET_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$MOGNET_NODE_ENGINE_RANGE" <<'NODE'
@@MOGNET_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for MOGNET_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$MOGNET_NODE_BIN/node" ]; then
        PATH="$MOGNET_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@MOGNET_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
MOGNET_NODE_SCRIPT_PATH=@@MOGNET_NODE_SCRIPT_PATH@@
if [ -n "$MOGNET_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$MOGNET_NODE_SCRIPT_PATH" "$@"
fi
...
require_installed_t3_cli() {
  ...
  printf 'Remote host installed %s but npm produced no t3 executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@MOGNET_PACKAGE_SPEC@@ >&2
  return 1
}
if command -v npx >/dev/null 2>&1; then
  require_installed_t3_cli npx --yes --package @@MOGNET_PACKAGE_SPEC@@ || exit 1
  exec npx --yes @@MOGNET_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  require_installed_t3_cli npm exec --yes --package @@MOGNET_PACKAGE_SPEC@@ || exit 1
  exec npm exec --yes @@MOGNET_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the t3 CLI and could not install @@MOGNET_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@MOGNET_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.mognet/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.mognet"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-mognet.sh"
RUNNER_NEXT="$STATE_DIR/run-mognet.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@MOGNET_DEFAULT_REMOTE_PORT@@" "@@MOGNET_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@MOGNET_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@MOGNET_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@MOGNET_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@MOGNET_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@MOGNET_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@MOGNET_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env MOGNET_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@MOGNET_READY_TIMEOUT_MS@@"; then
    printf 'Remote Mognet server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.mognet/ssh-launch/@@MOGNET_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.mognet"
RUNNER_FILE="$STATE_DIR/run-mognet.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  ...
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      MOGNET_PACKAGE_SPEC: packageSpec,
      MOGNET_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      MOGNET_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}
...
export function buildRemoteNodeEnvScript(input?: RemoteT3RunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      MOGNET_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      MOGNET_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}
...
export function buildRemoteLaunchScript(input?: RemoteT3RunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    MOGNET_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    MOGNET_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteT3RunnerScript(input)),
    MOGNET_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    MOGNET_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    MOGNET_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    MOGNET_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    MOGNET_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    MOGNET_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    MOGNET_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}
...
export function buildRemotePairingScript(
  ...
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    MOGNET_STATE_KEY: remoteStateKey(target),
    MOGNET_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteT3RunnerScript(input)),
  });
}
