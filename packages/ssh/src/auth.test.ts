...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      assert.equal(env.MOGNET_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "mognet");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\mognet-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\mognet-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
