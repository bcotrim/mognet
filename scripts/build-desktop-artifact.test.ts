...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches the bundled Mognet splash and favicon branding for nightly versions", () => {
  ...
  it("stages the resource monitor as an external executable resource", () => {
    ...
    assert.equal(resourceMonitorExecutableName("mac"), "mognet-resource-monitor");
    assert.equal(resourceMonitorExecutableName("win"), "mognet-resource-monitor.exe");
  });
...
it.effect("rebases packaged links into the isolated tree", () =>
  Effect.gen(function* () {
    ...
    const root = yield* fs.makeTempDirectoryScoped({ prefix: "mognet-copy-symlinks-" });
