...
it("batches codesign calls without changing existing signing options", async () => {
  const options = {
    app: "/tmp/Mognet.app",
    identity: "Developer ID Application: Mognet",
    keychain: "/tmp/mognet.keychain",
    provisioningProfile: "/tmp/mognet.provisionprofile",
    optionsForFile: () => ({
      entitlements: "/tmp/mognet.entitlements.plist",
