...
This guide is for people who want to use more than one Claude setup in Mognet.
...
In Mognet Settings, your Claude provider can stay like this:
...
Then add another Claude provider in Mognet:
...
Mognet only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so Mognet keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in Mognet Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Mognet stores the value as a server secret and does not
...
Mognet does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in Mognet.
