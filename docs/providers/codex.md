...
This guide is for people who want to use more than one Codex account in Mognet.
...
- both accounts can see the same Mognet/Codex sessions
...
In Mognet Settings, name it something obvious:
...
In Mognet Settings, add another Codex provider:
...
Mognet shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Mognet treats it
