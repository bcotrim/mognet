...
Mognet has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.mognet/userdata/logs/server.trace.ndjson`).
...
```bash
export MOGNET_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export MOGNET_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export MOGNET_OTLP_SERVICE_NAME=mognet-local
```

Optional:

```bash
export MOGNET_TRACE_MIN_LEVEL=Info
export MOGNET_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `MOGNET_OTLP_*`.

macOS app bundle example:

```bash
MOGNET_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
MOGNET_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
MOGNET_OTLP_SERVICE_NAME=mognet-desktop \
"/Applications/Mognet.app/Contents/MacOS/Mognet"
```
...
```bash
tail -f "$MOGNET_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$MOGNET_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$MOGNET_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `mognet-local`, `mognet-dev`, or `mognet-desktop`
...
- `MOGNET_OTLP_TRACES_URL` was not set
...
- `MOGNET_TRACE_FILE`: override trace file path
- `MOGNET_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `MOGNET_TRACE_MAX_FILES`: rotated file count, default `10`
- `MOGNET_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `MOGNET_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `MOGNET_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `MOGNET_OTLP_TRACES_URL`: OTLP trace endpoint
- `MOGNET_OTLP_METRICS_URL`: OTLP metric endpoint
- `MOGNET_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `MOGNET_OTLP_SERVICE_NAME`: service name, default `mognet-server`
