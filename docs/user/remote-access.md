...
Use this when you want to connect to a Mognet server from another device such as a phone, tablet, or separate desktop app.
...
`t3 pair` finds the running server (the shared `~/.mognet` install, or the current worktree's dev server when run inside one), issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your phone.
...
If no server is running, `t3 pair` points you at `npx t3 serve`.
...
- HTTPS/WSS-compatible endpoints are useful when your client requires secure browser transport.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address.
...
For browser clients served over HTTPS, prefer an HTTPS Tailnet or another HTTPS endpoint. Plain HTTP LAN endpoints work from clients that can open those URLs directly.
...
Use this when you want the desktop app to start or reuse Mognet on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote Mognet server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual Mognet server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.mognet/ssh-launch/<host-key>/`, starts or reuses a remote Mognet server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. Mognet uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Mognet first checks whether `node` is already available on `PATH`. If it is missing, the launcher tries common non-interactive shell locations and version-manager shims/activation hooks:
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Mognet uses:
...
- Pairing links keep the credential in the URL hash when possible, but it can still be exposed through browser history, screenshots, logs, or copy/paste.
