...
Load this reference only when inspecting or seeding local Mognet state directly.
...
```bash
node apps/server/scripts/mognet-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/mognet-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/mognet-seed.sql
```
