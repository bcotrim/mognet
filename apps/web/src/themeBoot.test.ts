...
const THEME_STORAGE_KEY = "mognet:theme";
...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "Mognet follows a dark OS",
      storage: { [THEME_STORAGE_KEY]: "mognet", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "an explicit global dark mode applies to Mognet",
      storage: {
        [THEME_STORAGE_KEY]: "mognet",
    ...
    {
      name: "a legacy mognet-grove preference resolves through the alias",
      storage: { [THEME_STORAGE_KEY]: "mognet-grove", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "legacy mognet-dark resolves to dark Mognet",
      storage: { [THEME_STORAGE_KEY]: "mognet-dark" },
      prefersDark: true,
    },
    {
  ...
  it("marks built-in and custom themes on the document element", () => {
    ...
    expect(chat.themeId).toBe("mognet");
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "mognet:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("mognet");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(MOGNET_THEME, "light")!.canvas,
    );
  });
  ...
  it("resolves a legacy-prefixed mix half onto the renamed theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "mognet",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "mognet:theme-halves:v1": JSON.stringify({ dark: "mognet-grove" }),
      },
      prefersDark: true,
    });
    expect(boot.isDark).toBe(true);
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "mognet",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "mognet:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("mognet");
    expect(boot.isDark).toBe(true);
  });
