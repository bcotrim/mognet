...
export type ComposerSlashCommand = "model" | "plan" | "default" | "interactive-review";
...
export function parseStandaloneComposerSlashCommand(
  ...
  const match = /^\/(plan|default|interactive-review|tour)\s*$/i.exec(text.trim());
  if (!match) {
    return null;
  }
  const command = match[1]?.toLowerCase();
  if (command === "plan") return "plan";
  if (command === "interactive-review" || command === "tour") return "interactive-review";
  return "default";
}
