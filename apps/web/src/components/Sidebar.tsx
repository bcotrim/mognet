...
import { SidebarPrimaryNavigation } from "./LegacySidebar";

// Settled-tail paging: recent history is the common lookup; the deep tail
// stays behind an explicit Show more.
const SETTLED_TAIL_INITIAL_COUNT = 10;
const SETTLED_TAIL_PAGE_COUNT = 25;
const SETTLED_SHELF_EXPANDED_KEY = "mognet:sidebar:settled-expanded";
const SNOOZED_SHELF_EXPANDED_KEY = "mognet:sidebar:snoozed-expanded";
...
export default function Sidebar() {
  ...
  const threadSearchResultOrderKey = threadSearchResults
    .map((thread) => scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)))
    .join("\0");
  const displayedPinnedThreads = pinnedThreads;
  const displayedActiveThreads = activeThreads;
  const displayedSnoozedThreads = snoozedThreads;
  const displayedSettledThreads = settledThreads;
