...
function ChatViewContent(props: ChatViewProps) {
  ...
  const terminalPanelOpenByThreadRef = useRef<Record<string, boolean>>({});
  ...
  const serverThreadError = activeServerThread?.session?.lastError ?? null;
  const threadError = isServerThread
    ? resolveThreadError({
        localError: localServerError,
        serverError: serverThreadError,
  ...
  const activeKnownTerminalIds = useMemo(
    () => [...new Set(activeServerOrderedTerminalIds)],
  ...
  const isProjectlessChat =
    activeThread?.projectId === STANDALONE_CHAT_PROJECT_ID || activeProject?.kind === "standalone";
  const hasWorkspaceProject = activeProject !== null && !isProjectlessChat;
  ...
  const activeProjectKey = hasWorkspaceProject
  ...
  const configuredPreviewUrls = useMemo(
    () => (hasWorkspaceProject ? getConfiguredPreviewUrls(activeProject?.scripts) : []),
    [activeProject?.scripts, hasWorkspaceProject],
  );

  useEffect(() => {
    if (!activeThreadRef || !activeEnvironmentBootstrapComplete) return;
    useRightPanelStore.getState().reconcileFileSurfaces(activeThreadRef, hasWorkspaceProject);
  }, [activeEnvironmentBootstrapComplete, activeThreadRef, hasWorkspaceProject]);
  ...
  const gitCwd = hasWorkspaceProject
    ? projectScriptCwd({
  ...
  const [resumeCompactionPermanentlyDismissed, setResumeCompactionPermanentlyDismissed] =
    useLocalStorage(
      `mognet:resume-compaction-dismissed:${environmentId}:${activeProviderInstanceId ?? "claudeAgent"}`,
      false,
      Schema.Boolean,
    );
  ...
  const isGitRepo = hasWorkspaceProject ? (gitStatusQuery.data?.isRepo ?? true) : false;
  ...
  }, [activeThread, isServerThread, routeThreadKey, serverThreadError, setThreadError]);
  ...
  const runProjectScript = useCallback(
    async (script: ProjectScript) => {
      ...
      const preferredTerminalSurface =
      ...
      const baseTerminalId =
        preferredTerminalSurface?.activeTerminalId ??
        activeKnownTerminalIds[0] ??
        DEFAULT_THREAD_TERMINAL_ID;
      ...
      if (shouldCreateNewTerminal || !preferredTerminalSurface) {
        rightPanelStore.openTerminal(activeThreadRef, targetTerminalId);
      } else {
        rightPanelStore.activateSurface(activeThreadRef, preferredTerminalSurface.id);
        rightPanelStore.activateTerminal(
          activeThreadRef,
          preferredTerminalSurface.id,
          targetTerminalId,
        );
      }
  ...
  const togglePreviewPanel = useCallback(() => {
    if (!activeThreadRef || !hasWorkspaceProject || !isPreviewSupportedInRuntime()) return;
    if (previewPanelOpen) {
      useRightPanelStore.getState().close(activeThreadRef);
      return;
    }
  ...
  const addTerminalSurface = useCallback(() => {
    if (!activeThreadRef || !activeThreadId || !hasWorkspaceProject) return;
  ...
  const toggleTerminalPanel = useCallback(() => {
    ...
    const existingTerminalSurface = rightPanelState.surfaces.find(
      (surface): surface is Extract<RightPanelSurface, { kind: "terminal" }> =>
        surface.kind === "terminal",
    );
    if (existingTerminalSurface) {
      useRightPanelStore.getState().activateSurface(activeThreadRef, existingTerminalSurface.id);
      setTerminalFocusRequestId((value) => value + 1);
      return;
    }
    addTerminalSurface();
  ...
  const cleanupRightPanelSurfaces = useCallback(
    ...
    [activeThreadRef, activePreviewState.sessions, closePreview, closeTerminalMutation],
  );
  ...
  // the sent message can stay near the top. Mognet only owns streaming adjustments
  ...
  useEffect(() => {
    if (!activeThread?.id || terminalPanelOpen) return;
  ...
  }, [activeThread?.id, focusComposer, terminalPanelOpen]);
  ...
  const handleSwitchCheckoutToThread = useCallback(async () => {
    if (!gitStatusCwd || !activeThread || !checkoutBranchMismatch || isRestoringThreadBranch) {
      return;
    }
    ...
    if (nextBranch !== activeThread.branch) {
      const updateResult = await updateThreadMetadata({
        environmentId,
        input: { threadId: activeThread.id, branch: nextBranch },
      });
  ...
  }, [activeThreadId, terminalPanelOpen]);

  useEffect(() => {
    if (!activeThreadKey) return;
    const previous = terminalPanelOpenByThreadRef.current[activeThreadKey] ?? false;
    const current = terminalPanelOpen;

    if (!previous && current) {
      terminalPanelOpenByThreadRef.current[activeThreadKey] = current;
      setTerminalFocusRequestId((value) => value + 1);
      return;
    ...
    terminalPanelOpenByThreadRef.current[activeThreadKey] = current;
  }, [activeThreadKey, focusComposer, terminalPanelOpen]);
  ...
  const onSend = async (
    ...
    if (!hasWorkspaceProject && !isProjectlessChat) return;
    ...
    if (failure === null && turnAttachmentsResult._tag === "Success") {
      const bootstrap = shouldIncludeTurnStartBootstrap({
        isLocalDraftThread,
        hasBaseBranchForWorktree: baseBranchForWorktree !== null,
      })
        ? {
            ...(isProjectlessChat && isLocalDraftThread ? { ensureStandaloneProject: true } : {}),
            ...(isLocalDraftThread
              ? {
                  createThread: {
                    projectId: isProjectlessChat ? STANDALONE_CHAT_PROJECT_ID : activeProject!.id,
                    title,
                    modelSelection: threadCreateModelSelection,
                    runtimeMode,
                    interactionMode,
                    branch: isProjectlessChat ? null : activeThreadBranch,
                    worktreePath: isProjectlessChat ? null : activeThread.worktreePath,
  ...
  const rightPanelContent = activeThreadRef ? (
    hasWorkspaceProject && activeRightPanelSurface?.kind === "preview" ? (
    ...
    ) : hasWorkspaceProject && activeRightPanelSurface?.kind === "terminal" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Mognet server to browse pull requests."
      />
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      >
        ...
        >
          {!rightPanelOpen || !hasWorkspaceProject ? panelLayoutControls : null}
          <ChatHeader
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          ...
          >
            ...
            >
              ...
              >
                <div className="group/composer-stack pointer-events-auto relative z-10 isolate">
                  {isDraftHeroState ? (
                  ...
                  >
                    <div
                      ...
                      className={cn(
                        "chat-composer-glass-host relative z-10 mx-auto w-full max-w-3xl rounded-[22px]",
            ...
            {pullRequestDialogState && hasWorkspaceProject ? (
      ...
      {!shouldUseRightPanelSheet && rightPanelOpen && activeThreadRef ? (
        <RightPanelTabs
          ...
          terminalAvailable={hasWorkspaceProject}
