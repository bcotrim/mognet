...
import { useMognetProjectFileState } from "../../hooks/useMognetProjectFileScripts";
...
function ProjectDetail({
  ...
  const textGenerationSelection = representative.textGenerationModelSelection;
  const resolvedTextGenerationSelection = resolveDefaultProviderModelSelection(
    serverProviders,
    textGenerationSelection,
  );
  const textGenerationEntry = instanceEntries.find(
    (entry) => entry.instanceId === resolvedTextGenerationSelection?.instanceId,
  );
  const setTextGenerationModel = useCallback(
    (selection: ModelSelection) =>
      void updateAllMembers(
        { textGenerationModelSelection: selection },
        "Failed to update text generation model",
      ),
    [updateAllMembers],
  );

  // ----- new-thread workspace mode -----
  const storedEnvMode = representative.defaultThreadEnvMode;
  const setDefaultThreadEnvMode = useCallback(
    (mode: ProjectDefaultThreadEnvMode) =>
      void updateAllMembers(
  ...
  const [defaultBranchDraft, setDefaultBranchDraft] = useState(representative.defaultBranch ?? "");
  useEffect(() => {
    setDefaultBranchDraft(representative.defaultBranch ?? "");
  }, [representative.defaultBranch]);
  const persistDefaultBranch = useCallback(() => {
    const defaultBranch = defaultBranchDraft.trim() || null;
    if (defaultBranch === representative.defaultBranch) return;
    setDefaultBranchDraft(defaultBranch ?? "");
    void updateAllMembers({ defaultBranch }, "Failed to update default branch");
  }, [defaultBranchDraft, representative.defaultBranch, updateAllMembers]);
  const setNewWorktreesStartFromOrigin = useCallback(
    (newWorktreesStartFromOrigin: boolean) =>
      void updateAllMembers(
        { newWorktreesStartFromOrigin },
        "Failed to update worktree origin preference",
      ),
    [updateAllMembers],
  );
  ...
  const mognetFile = useMognetProjectFileState(
    representative.environmentId,
    representative.workspaceRoot,
  );
  const importableScripts = useMemo(
    () =>
      mognetFile.scripts.filter(
        (fileScript) =>
          !scripts.some(
            (script) =>
  ...
  const importFileScript = useCallback(
    async (fileScript: MognetProjectFileScript) => {
  ...
  return (
    <SettingsPageContainer className="gap-10">
      ...
      <SettingsSection title="Model">
        <SettingsRow
          id="project-text-generation-model"
          title="Text generation model"
          description="Used for generated thread titles, worktree branch names, and similar project text."
          control={
            resolvedTextGenerationSelection && textGenerationEntry ? (
              <div className="flex flex-wrap items-center justify-end gap-1.5">
                <ProviderModelPicker
                  activeInstanceId={resolvedTextGenerationSelection.instanceId}
                  model={resolvedTextGenerationSelection.model}
                  lockedProvider={null}
                  ...
                  onInstanceModelChange={(instanceId, model) => {
                    setTextGenerationModel(createModelSelection(instanceId, model));
                  }}
                />
                <TraitsPicker
                  provider={textGenerationEntry.driverKind as ProviderDriverKind}
                  models={textGenerationEntry.models}
                  model={resolvedTextGenerationSelection.model}
                  prompt=""
                  onPromptChange={() => {}}
                  modelOptions={resolvedTextGenerationSelection.options ?? []}
                  ...
                  onModelOptionsChange={(nextOptions) => {
                    setTextGenerationModel(
                      createModelSelection(
                        resolvedTextGenerationSelection.instanceId,
                        resolvedTextGenerationSelection.model,
                        nextOptions,
                      ),
                    );
                  }}
      ...
      <SettingsSection title="New threads">
        <SettingsRow
          ...
          title="Workspace"
          description="Where new threads in this project start. Applies to every checkout in this group."
          control={
            <Select
              value={storedEnvMode}
              onValueChange={(value) => {
            ...
            >
              <SelectTrigger aria-label="New-thread workspace">
                <SelectValue>{resolveEnvModeLabel(storedEnvMode)}</SelectValue>
              </SelectTrigger>
              <SelectPopup align="end" alignItemWithTrigger={false}>
        ...
        {storedEnvMode === "worktree" ? (
          <SettingsRow
            ...
            control={
              <Switch
                checked={representative.newWorktreesStartFromOrigin}
                onCheckedChange={(checked) => setNewWorktreesStartFromOrigin(Boolean(checked))}
      ...
      >
        ...
        {mognetFile.status === "invalid" ? (
          <SettingsRow
            title="mognet.json is invalid"
            description="A mognet.json exists at the workspace root but fails to parse, so every script and icon it declares is ignored. Check the JSON syntax and icon values."
            className="text-warning"
          />
        ) : null}
        {importableScripts.length > 0 ? (
          <SettingsRow
            title="Import from mognet.json"
            description={`${importableScripts.length} script${importableScripts.length === 1 ? "" : "s"} declared in this repo's mognet.json ${importableScripts.length === 1 ? "is" : "are"} not set up yet.`}
            control={
