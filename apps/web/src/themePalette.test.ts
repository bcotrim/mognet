...
describe("theme files", () => {
  it("derives a readable palette from extreme simple-editor colors", () => {
    ...
    // Status colors fall back to Mognet's standard red and amber rather than
  ...
  it("canonicalizes the explicitly exported theme", () => {
    const serialized = serializeThemeFile({
      ...MOGNET_THEME,
      colors: { ...MOGNET_THEME.colors, accent: "hsl(263 70% 58%)" },
    });
    expect(JSON.parse(serialized)).toMatchObject({
      version: THEME_FILE_VERSION,
      id: MOGNET_THEME.id,
      name: MOGNET_THEME.label,
      appearance: "light",
  ...
  it("suppresses sidebar artwork during a live custom-theme preview", () => {
    ...
    applyThemeColorPreview(MOGNET_THEME.colors, "light");
  ...
  it("keeps optional light and dark palettes under one theme id", () => {
    ...
    expect(getThemeModes(MOGNET_THEME)).toEqual(["light", "dark"]);
    expect(resolveThemeAppearance(MOGNET_THEME.id, true, true)).toBe("dark");
    expect(resolveDesktopTheme(MOGNET_THEME.id, true)).toBe("system");
    expect(resolveThemeAppearance(MOGNET_THEME.id, false, false, "dark")).toBe("dark");
    expect(resolveDesktopTheme(MOGNET_THEME.id, false, "dark")).toBe("dark");
  ...
  it("keeps the Mognet palette faithful and readable", () => {
    expectThemeColors(MOGNET_THEME.colors, {
      canvas: "#fdf7fd",
      chrome: "#fdf7fd",
    ...
    expectThemeColors(MOGNET_THEME.variants!.dark!, {
      canvas: "#1f1a24",
      chrome: "#1f1a24",
      surface: "#29232d",
    ...
    for (const mode of ["light", "dark"] as const) {
      const colors = getThemeColorsForMode(MOGNET_THEME, mode)!;
      expect(contrastRatio(colors.text, colors.canvas)).toBeGreaterThanOrEqual(7);
      expect(contrastRatio(colors.textMuted, colors.canvas)).toBeGreaterThanOrEqual(4.5);
      expect(contrastRatio(colors.messageForeground, colors.messageSurface)).toBeGreaterThanOrEqual(
        4.5,
  ...
  it("includes the dual-mode maintainer themes", () => {
    for (const theme of [MOGNET_THEME, GROVE_THEME, OCEAN_THEME, EMBER_THEME, IRIS_THEME]) {
...
describe("stored theme preferences", () => {
  ...
  it("resolves the legacy mognet-dark preference to dark Mognet", () => {
    expect(getThemeDefinition("mognet-dark")).toBe(MOGNET_THEME);
    expect(getThemePreferenceMode("mognet-dark")).toBe("dark");
    expect(resolveThemeAppearance("mognet-dark", true, false)).toBe("dark");
    expect(resolveDesktopTheme("mognet-dark", false)).toBe("dark");
    expect(isKnownThemePreference("mognet-dark")).toBe(true);
  });

  it("resolves legacy mognet-prefixed ids onto the renamed themes", () => {
    for (const [legacy, theme] of [
      ["mognet-grove", GROVE_THEME],
      ["mognet-ocean", OCEAN_THEME],
      ["mognet-ember", EMBER_THEME],
      ["mognet-iris", IRIS_THEME],
    ] as const) {
    ...
    expect(canonicalThemePreference("mognet-dark")).toBe("mognet-dark");
    // A stored mix that predates the rename resolves to the new ids.
    expect(
      parseThemeHalves(JSON.stringify({ light: "mognet-ocean", dark: "mognet-grove" })),
    ).toEqual({
  ...
  it("recognizes only preferences the runtime can render", () => {
    for (const preference of ["light", "dark", "system", MOGNET_THEME.id, GROVE_THEME.id]) {
...
describe("singleAppearanceOf", () => {
  it("reports the only half a theme can claim, and null for a pair", () => {
    const { variants: _pair, ...base } = MOGNET_THEME;
    expect(singleAppearanceOf({ ...base, id: "x", appearance: "dark" })).toBe("dark");
    expect(singleAppearanceOf(MOGNET_THEME)).toBe(null);
  });
});
