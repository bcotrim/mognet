...
describe("theme files", () => {
  ...
  it("serializes a theme back into the importable file shape", () => {
    const serialized = serializeThemeFile(MOGNET_THEME);
  ...
  it("publishes sidebar artwork changes from the live theme preview", () => {
    ...
    applyThemeColorPreview(MOGNET_THEME.colors, "light", true);
  ...
  it("keeps the Mognet palette faithful and readable", () => {
    expect(MOGNET_THEME.colors).toMatchObject({
      canvas: "#fdf7fd",
      chrome: "#fdf7fd",
    ...
    expect(MOGNET_THEME.variants?.dark).toMatchObject({
      canvas: "#1f1a24",
      chrome: "#1f1a24",
      surface: "#29232d",
  ...
  it("includes the dual-mode maintainer themes", () => {
    for (const theme of [MOGNET_THEME, GROVE_THEME, OCEAN_THEME, EMBER_THEME, IRIS_THEME]) {
      ...
      for (const mode of ["light", "dark"] as const) {
        ...
        expect(
          contrastRatio(colors!.messageActionForeground, colors!.messageAction),
        ).toBeGreaterThanOrEqual(theme === MOGNET_THEME ? 3 : 4.5);
      }
    }
  });
