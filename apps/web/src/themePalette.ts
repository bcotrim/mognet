...
export const MOGNET_THEME_ID = "mognet" as const;
export const MOGNET_THEME_LABEL = "Mognet";
...
export const CUSTOM_THEMES_STORAGE_KEY = "mognet:themes:v1";
export const THEME_FOLLOW_SYSTEM_STORAGE_KEY = "mognet:theme-follow-system";
export const THEME_APPEARANCE_MODE_STORAGE_KEY = "mognet:theme-appearance-mode";
export const THEME_HALVES_STORAGE_KEY = "mognet:theme-halves:v1";

const LEGACY_MOGNET_DARK_THEME_ID = "mognet-dark";
...
// Earlier builds shipped every maintainer theme under a mognet- prefix; only the
...
const LEGACY_THEME_ID_ALIASES: Readonly<Record<string, string>> = {
  [LEGACY_MOGNET_DARK_THEME_ID]: MOGNET_THEME_ID,
  "mognet-grove": GROVE_THEME_ID,
  "mognet-ocean": OCEAN_THEME_ID,
  "mognet-ember": EMBER_THEME_ID,
  "mognet-iris": IRIS_THEME_ID,
};
...
export function canonicalThemePreference(theme: string): string {
  return theme === LEGACY_MOGNET_DARK_THEME_ID ? theme : normalizeThemeId(theme);
}
...
// Older builds stored the dark Mognet palette as a separate theme. Keep
...
function legacyThemeMode(theme: ThemePreference): ThemeAppearance | null {
  return theme === LEGACY_MOGNET_DARK_THEME_ID ? "dark" : null;
}
 ...
 */
// Measured from the live Mognet default theme. Translucent chat surfaces are
// flattened over --chat-background so this opaque palette reproduces the
// pixels users see after Mognet's blur and noise layers are composited.
// Foreground pairs deviate where necessary to keep normal text at WCAG AA.
const MOGNET_LIGHT_COLORS: ThemeColors = {
  canvas: "#fdf7fd",
  // Mognet's workspace header belongs to the chat panel, so keep it seamless
  // with the light chat canvas rather than mapping it to Mognet's outer shell.
  chrome: "#fdf7fd",
  toolbar: "#fdf7fd",
  toolbarForeground: "#501854",
  toolbarBorder: "#efbdeb",
  // Mognet's light chrome controls sit on its pale gradient-noise surface,
  ...
  // Mognet uses a light lavender code surface in light mode. Keeping the
  // dark plum pair here also leaked the dark palette into Mognet's diffs.
  codeBackground: "#f5ecf9",
  codeForeground: "#673c8b",
  // The live sidebar is transparent over Mognet's outer shell. Use that
...
const MOGNET_DARK_COLORS: ThemeColors = {
  canvas: "#1f1a24",
  // Mognet's workspace header belongs to the chat panel, so keep it seamless
  // with the canvas rather than mapping it to Mognet's outer shell.
  chrome: "#1f1a24",
  toolbar: "#1f1a24",
  ...
  // Cards and panels stay in Mognet's plum surface family. Near-black here
  ...
  // Diffs and file previews are full workspace surfaces in Mognet. Keep them
  ...
  // Mognet draws the chat panel edge in this muted pink. The resize rail uses
...
/**
 * The palette Mognet wears with no theme installed, captured from the app's
 ...
 */
const MOGNET_LIGHT_THEME_COLORS: ThemeColors = {
  canvas: "#fcfcfc",
  chrome: "#fcfcfc",
  toolbar: "#fcfcfc",
...
const MOGNET_DARK_THEME_COLORS: ThemeColors = {
  canvas: "#0a0a0a",
  chrome: "#0a0a0a",
  toolbar: "#0a0a0a",
...
/**
 * The standard Mognet look as a theme palette, for seeding a new theme when
 * no theme is installed. Distinct from {@link getDefaultThemeColors}, which
 * carries the flagship Mognet palette used to fill roles omitted by theme
 * files.
 */
export function getStandardThemeColors(appearance: ThemeAppearance): ThemeColors {
  if (appearance === "dark") {
    return (standardDarkThemeColors ??= decodeThemeColors(MOGNET_DARK_THEME_COLORS));
  }
  return (standardLightThemeColors ??= decodeThemeColors(MOGNET_LIGHT_THEME_COLORS));
}
...
/**
 * The status colors Mognet shows without a theme, read from the app's own
...
export const MOGNET_THEME: ThemeDefinition = {
  id: MOGNET_THEME_ID,
  label: MOGNET_THEME_LABEL,
  appearance: "light",
  colors: decodeThemeColors(MOGNET_LIGHT_COLORS),
  variants: {
    dark: decodeThemeColors(MOGNET_DARK_COLORS),
  },
  sidebarArtwork: true,
};
...
export function getDefaultThemeColors(appearance: ThemeAppearance): ThemeColors {
  return appearance === "dark" ? MOGNET_THEME.variants!.dark! : MOGNET_THEME.colors;
}

/**
 * A companion action color in the Mognet mold. This gives send buttons,
...
function themeActionColors(
  ...
  const towardOpposite =
    foreground === THEME_LIGHT_FOREGROUND || foreground === THEME_WHITE_FOREGROUND
      ...
      : THEME_WHITE_FOREGROUND;
  return {
    messageAction: toCanonicalThemeColor(action) ?? themeRgbToThemeColor(rgb),
    messageActionForeground: themeRgbToThemeColor(foreground),
    messageActionHover: themeRgbToThemeColor(mixThemeRgbColors(rgb, towardOpposite, 0.12)),
  };
}
