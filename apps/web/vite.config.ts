...
const isSingleOriginDev = process.env.MOGNET_SINGLE_ORIGIN_DEV === "1";
...
// round trip per import level in unbundled dev); MOGNET_BUNDLED_DEV=0 opts out.
...
function resolveDevProxyTarget(
  ...
  // MOGNET_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.MOGNET_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.MOGNET_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
