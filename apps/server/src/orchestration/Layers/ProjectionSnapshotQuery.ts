...
const ProjectionProjectDbRowSchema = ProjectionProject.mapFields(
  Struct.assign({
    ...
    newWorktreesStartFromOrigin: Schema.Number,
    textGenerationModelSelection: Schema.fromJsonString(ModelSelection),
...
const ProjectionThreadDbRowSchema = ProjectionThread.mapFields(
  Struct.assign({
    ...
    origin: Schema.optionalKey(Schema.NullOr(Schema.fromJsonString(OrchestrationThreadOrigin))),
  }),
);
...
function mapProjectShellRow(
  ...
  return {
    ...
    newWorktreesStartFromOrigin: row.newWorktreesStartFromOrigin === 1,
    textGenerationModelSelection: row.textGenerationModelSelection,
    scripts: row.scripts,
    ...
    execute: () =>
      sql`
        SELECT
          ...
          new_worktrees_start_from_origin AS "newWorktreesStartFromOrigin",
          text_generation_model_selection_json AS "textGenerationModelSelection",
            ...
            : repositoryIdentityResolver.resolve(option.value.workspaceRoot).pipe(
                Effect.map((repositoryIdentity) =>
                  Option.some({
                    ...
                    defaultBranch: option.value.defaultBranch,
                    defaultThreadEnvMode: option.value.defaultThreadEnvMode,
                    newWorktreesStartFromOrigin: option.value.newWorktreesStartFromOrigin === 1,
                    textGenerationModelSelection: option.value.textGenerationModelSelection,
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        worktreePath: threadRow.value.worktreePath,
        ...(threadRow.value.origin ? { origin: threadRow.value.origin } : {}),
