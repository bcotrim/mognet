...
import * as MognetProjectFileLoader from "./project/MognetProjectFileLoader.ts";
...
const buildAppUnderTest = (options?: {
  ...
  layers?: {
    ...
    scheduledTasks?: Partial<ScheduledTasks.ScheduledTasks["Service"]>;
    ...
    const servedRoutesBaseLayer = HttpRouter.serve(makeRoutesLayer, {
      ...
      Layer.provide(
        Layer.mock(ScheduledTasks.ScheduledTasks)({
          ...
          create: () => Effect.die("ScheduledTasks.create not stubbed in this test"),
          update: () => Effect.die("ScheduledTasks.update not stubbed in this test"),
          delete: () => Effect.die("ScheduledTasks.delete not stubbed in this test"),
          runNow: () => Effect.die("ScheduledTasks.runNow not stubbed in this test"),
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const url = yield* getHttpServerUrl("/.well-known/mognet/environment");
      ...
      Effect.gen(function* () {
        ...
        const tokenResponse = yield* fetchEffect(tokenUrl, {
          ...
          body: new URLSearchParams({
            ...
            subject_token_type: "urn:mognet:params:oauth:token-type:environment-bootstrap",
  ...
  for (const desktopOrigin of ["mognet://app", "mognet-dev://app"]) {
  ...
  it.effect("routes websocket rpc shell.openInTerminal", () =>
    Effect.gen(function* () {
      let openedInput: { cwd: string; terminal: ExternalTerminalId } | null = null;
      yield* buildAppUnderTest({
        layers: {
          externalLauncher: {
      ...
      assert.deepEqual(openedInput, { cwd: "/tmp/project", terminal: "ghostty" });
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );

  it.effect("routes websocket rpc shell.openInTerminal errors", () =>
    Effect.gen(function* () {
      const externalLauncherError = new ExternalLauncherTerminalCommandNotFoundError({
        terminal: "ghostty",
        command: "ghostty",
      });
      yield* buildAppUnderTest({
      ...
      Effect.gen(function* () {
        ...
        const refreshStatus = vi.fn((_: string) =>
          Effect.succeed({
            ...
            hasPrimaryRemote: true,
            isDefaultRef: false,
            refName: "mognet/bootstrap-refName",
            hasWorkingTreeChanges: false,
            workingTree: {
              files: [],
              insertions: 0,
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          ...
          newRefName: "mognet/bootstrap-refName",
          baseRefName: "main",
          path: null,
        });
