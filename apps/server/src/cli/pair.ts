 ...
 * own `.mognet` is checked first (matching dev-runner precedence); otherwise the
 * shared Mognet home. `--tailscale` publishes the server over Tailscale Serve
...
import { resolveWorktreeMognetHome } from "@t3tools/shared/devHome";
import {
...
const WELL_KNOWN_ENVIRONMENT_PATH = "/.well-known/mognet/environment";
...
export class NoRunningServerError extends Schema.TaggedErrorClass<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      "No running Mognet server found.",
...
export class ServesOtherEnvironmentError extends Schema.TaggedErrorClass<ServesOtherEnvironmentError>()(
  ...
  override get message(): string {
    return `Tailscale Serve on HTTPS port ${String(this.servePort)} already fronts a different Mognet server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
export class ServePortOccupiedError extends Schema.TaggedErrorClass<ServePortOccupiedError>()(
  ...
  override get message(): string {
    return `HTTPS port ${String(this.servePort)} on the tailnet already serves something that is not a Mognet server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
/**
 * Three outcomes, because they drive different decisions: a Mognet descriptor
 * (pair with it), nothing answering (safe to configure Tailscale Serve), or
 * something answering that is not a Mognet server (do NOT overwrite its mapping).
 */
type EnvironmentProbeResult =
  | { readonly _tag: "descriptor"; readonly descriptor: ExecutionEnvironmentDescriptor }
  | { readonly _tag: "unreachable" }
  | { readonly _tag: "not-a-mognet-server" };
...
const probeEnvironmentDescriptor = (
  ...
  Effect.gen(function* () {
    ...
    const descriptor = yield* HttpClientResponse.filterStatusOk(response).pipe(
      ...
      Effect.mapError(() => ({ _tag: "not-a-mognet-server" }) as const),
    );
...
const discoverPairTarget = Effect.fn("pair.discoverPairTarget")(function* (
  ...
  if (explicitBaseDir !== undefined && explicitBaseDir.trim().length > 0) {
    ...
    // Same precedence as dev-runner: inside a linked worktree its own `.mognet`
    ...
    const worktreeHome = yield* resolveWorktreeMognetHome(process.cwd());
    if (worktreeHome !== undefined) {
      bases.push(worktreeHome);
    }
    const envHome = yield* Config.string("MOGNET_HOME").pipe(Config.option);
...
const resolveTailscalePairingBase = Effect.fn("pair.resolveTailscalePairingBase")(
  function* (input: { readonly target: DiscoveredPairTarget; readonly servePort: number }) {
    ...
    if (existing._tag === "not-a-mognet-server") {
...
const mintPairingLink = Effect.fn("pair.mintPairingLink")(function* (input: {
  ...
  return yield* Effect.gen(function* () {
    ...
    return yield* environmentAuth.createPairingLink({
      ...
      label: Option.getOrElse(input.label, () => "Mognet pair"),
...
export const pairCommand = Command.make("pair", {
  ...
  Command.withDescription(
    "Mint a pairing token for a running Mognet server and print it as a QR code.",
  ),
