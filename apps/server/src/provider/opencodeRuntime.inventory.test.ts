...
it.layer(testLayer)("OpenCodeRuntime inventory", (it) => {
  ...
  it.effect("drops oversized CLI skill output without losing the model inventory", () =>
    Effect.gen(function* () {
      ...
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "mognet-opencode-inventory-" });
      ...
      yield* fs.writeFileString(
        ...
        [
          ...
          isWindows
            ? '"%MOGNET_TEST_NODE_BINARY%" "%MOGNET_TEST_OPENCODE_SCRIPT%" %*'
            : 'exec "$MOGNET_TEST_NODE_BINARY" "$MOGNET_TEST_OPENCODE_SCRIPT" "$@"',
          "",
        ].join("\n"),
      );
      if (!isWindows) {
      ...
      const inventory = yield* runtime.loadInventoryFromCli({
        ...
        environment: {
          ...hostEnvironment,
          MOGNET_TEST_NODE_BINARY: executablePath,
          MOGNET_TEST_OPENCODE_SCRIPT: scriptPath,
        },
      });
