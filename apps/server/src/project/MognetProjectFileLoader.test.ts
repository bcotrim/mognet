...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(MognetProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "mognet.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("MognetProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid mognet.json", () =>
      Effect.gen(function* () {
        const loader = yield* MognetProjectFileLoader.MognetProjectFileLoader;
    ...
    it.effect("returns none when mognet.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* MognetProjectFileLoader.MognetProjectFileLoader;
        const cwd = yield* makeTempDir;
