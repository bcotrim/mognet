...
function checkoutBranchName(input: {
  ...
  return `mognet/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function responseError(
  ...
  return Effect.gen(function* () {
    ...
    const collected = yield* collectResponseText(operation, response, DEFAULT_MAX_RESPONSE_BYTES);
...
function collectResponseText(
  ...
  return collectUint8StreamText({
    ...
    // Effect beta.78 can surface a failed ReadableStream as a defect. Preserve
    // that immediate cause while keeping the API failure typed.
