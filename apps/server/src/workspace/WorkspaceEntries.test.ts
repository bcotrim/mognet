...
it.layer(TestLayer, { excludeTestServices: true })("WorkspaceEntries", (it) => {
  ...
  describe("search", () => {
    ...
    it.effect("applies the file filter before limiting search results", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-file-limit-" });
        yield* writeTextFile(cwd, "src/index.ts");
    ...
    it.effect("answers an empty file-filtered query with a bounded file listing", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-empty-query-" });
        yield* writeTextFile(cwd, "src/index.ts");
        yield* writeTextFile(cwd, "README.md");
    ...
    it.effect("returns only directories for the directory filter", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-directory-filter-" });
        yield* writeTextFile(cwd, "src/index.ts");
  ...
  describe("searchContents", () => {
    it.effect("returns content matches with file paths, line numbers, and ranges", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-content-search-" });
        yield* writeTextFile(
          cwd,
          "src/shapes.ts",
    ...
    it.effect("honors case sensitivity and gitignore rules", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-content-ignore-", git: true });
    ...
    it.effect("filters whole-word matches by word boundaries without widening ranges", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-content-whole-word-" });
    ...
    it.effect("finds later whole-word matches in a file after rejected raw matches", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-content-late-whole-word-" });
    ...
    it.effect("treats astral-plane letters as whole word characters", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-content-astral-word-" });
    ...
    it.effect("matches punctuation-edged whole-word queries including adjacent occurrences", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-content-punctuation-" });
    ...
    it.effect("matches punctuation-edged regex queries as whole words", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-content-regex-punctuation-" });
    ...
    it.effect("caps matches per file so one dense file cannot fill the page", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-content-per-file-cap-" });
    ...
    it.effect("preserves regex escapes during case-insensitive searches", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-content-regex-" });
    ...
    it.effect("preserves invalid regex errors during case-insensitive searches", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-content-invalid-regex-" });
    ...
    it.effect("maps multi-byte lines to string-indexed ranges", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "mognet-workspace-content-multibyte-" });
