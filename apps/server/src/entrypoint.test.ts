...
const makeTempDir = () =>
  NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "mognet-entrypoint-test-"));
...
describe("isEntrypoint", () => {
  ...
  it("matches through a symlinked entrypoint, as npm and npx install it", () => {
    ...
    const link = NodePath.join(dir, "mognet");
    NodeFS.writeFileSync(real, "");
    NodeFS.symlinkSync(real, link);

    expect(
      isEntrypoint({
        // Node hands modules an already-resolved URL, which matters on macOS
        // where the temp directory itself is reached through a symlink.
