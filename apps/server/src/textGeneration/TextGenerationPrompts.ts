...
const INITIAL_THREAD_TITLE_PROMPT = `Generate a title that will help the user recognize this Mognet thread weeks later.
...
function regenerateThreadTitlePrompt(previousTitle: string): string {
  return `Regenerate the title for an existing Mognet thread so the user can recognize it weeks later.
...
export interface ReviewSnapshotPromptInput {
  ...
  fileContexts: ReadonlyArray<{
    ...
    hunkCount: number;
    truncated: boolean;
    anchors: ReadonlyArray<{ id: string; rangeLabel: string; diff: string }>;
  }>;
