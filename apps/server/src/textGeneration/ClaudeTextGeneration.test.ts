  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  }",
        "fi",
        'if [ -n "$MOGNET_FAKE_CLAUDE_CONFIG_DIR_MUST_BE" ] && [ "$CLAUDE_CONFIG_DIR" != "$MOGNET_FAKE_CLAUDE_CONFIG_DIR_MUST_BE" ]; then',
...
function withFakeClaudeEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    ...
    const previousConfigDirMustBe = process.env.MOGNET_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;
    ...
    yield* Effect.acquireRelease(
      Effect.sync(() => {
        ...
        if (input.configDirMustBe !== undefined) {
          process.env.MOGNET_FAKE_CLAUDE_CONFIG_DIR_MUST_BE = input.configDirMustBe;
        } else {
          delete process.env.MOGNET_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;
        }
      }),
      () =>
        Effect.sync(() => {
          ...
          if (previousConfigDirMustBe === undefined) {
            ...
            process.env.MOGNET_FAKE_CLAUDE_CONFIG_DIR_MUST_BE = previousConfigDirMustBe;
          }
        }),
    );
