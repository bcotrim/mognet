...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Mognet once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.mognet database. Use an isolated --base-dir.";
  }
}
...
export const mognetSqliteStateCommand = Command.make(
  "mognet-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Mognet base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Mognet SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(mognetSqliteStateCommand, { version: "0.0.0" }).pipe(
