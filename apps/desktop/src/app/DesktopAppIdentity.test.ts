...
const defaultEnvironmentInput = {
  ...
  processArch: "arm64",
  appVersion: "1.2.3",
  appPath: "/Applications/Mognet.app/Contents/Resources/app.asar",
  isPackaged: true,
  resourcesPath: "/Applications/Mognet.app/Contents/Resources",
          ...
          FileSystem.layerNoop({
            exists: (path) =>
              input.legacyPathProbeError
                ...
                : Effect.succeed(
                    input.legacyPathExists === true && path.includes("Mognet (Alpha)"),
                  ),
            readFileString: () =>
              Effect.succeed(input.packageJson ?? '{"mognetCommitHash":"abcdef1234567890"}'),
          }),
        ),
...
describe("DesktopAppIdentity", () => {
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/Mognet (Alpha)");
      }),
  ...
  it.effect("preserves failures while inspecting the legacy userData path", () => {
    const legacyPath = "/Users/alice/Library/Application Support/Mognet (Alpha)";
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Mognet"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Mognet");
      ...
      }),
      {
        ...
        environment: {
          env: {
            MOGNET_COMMIT_HASH: "0123456789abcdef",
          },
        },
