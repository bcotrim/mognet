    ...
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          ...
          MOGNET_DEV_REMOTE_SERVER_ENTRY_PATH: " /remote/server.mjs ",
          MOGNET_OTLP_TRACES_URL: " http://127.0.0.1:4318/v1/traces ",
          MOGNET_OTLP_EXPORT_INTERVAL_MS: "2500",
        },
      );
      ...
      assert.equal(environment.appUserModelId, "app.mognet.desktop.dev");
      assert.equal(environment.linuxWmClass, "mognet-dev");
      assert.deepEqual(
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          MOGNET_DESKTOP_APP_USER_MODEL_ID: " app.mognet.desktop.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "app.mognet.desktop.dev.local");
    }),
  );
